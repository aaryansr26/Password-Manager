...
parser.add_argument('option', help='(a)dd / (e)xtact / (g)enerate')
...
def inputAndValidateMasterPass():
...
def main():
    if args.option in ["add", "a"]:
        ...
        res = inputAndValidateMasterPass()
    ...
    if args.option in ["extract", "e"]:
        ...
        if res is not None:
            utils.retrieve.retrieve(res[0], res[1], search, decryptPassword=args.copy)
    
    if args.option in ["generate", "g"]:
        if args.length == None:
            printc("[red][!][/red] Specify length to generate (--length)")
            return
